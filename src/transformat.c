...
Format_Type format_type_for_name(string name) {
  
  for(size_t i=0;i<sizeof(assocs)/sizeof(assocs[0]);i++) {
    Assoc *assoc = &assocs[i];
    
    if(string_eq_cstr(name, assoc->name)) {
      return assoc->type;
    }
  }
...
Format_Type format_type_for_filename(string filename) {
  if(!filename.len) return FORMAT_TYPE_NONE;

  string last = filename;
  while(filename.len) {
    string part = string_chop_by_delim(&filename, '.');
    last = part;
  }
...
int main(int argc, char **argv) {
  ...
  if(in[0] == '-') {
    input_format = format_type_for_name(string_from(in + 1, strlen(in) - 1));
  ...
  if(in[0] == '-') {
    output_format = format_type_for_name(string_from(in + 1, strlen(in) - 1));
  ...
  if(input_format == FORMAT_TYPE_NONE) {
    input_format = format_type_for_filename(string_from_cstr(input));
  }
  if(input_format == FORMAT_TYPE_NONE) {
  ...
  if(output_format == FORMAT_TYPE_NONE) {
    output_format = format_type_for_filename(string_from_cstr(output));
  }
  if(output_format == FORMAT_TYPE_NONE) {
